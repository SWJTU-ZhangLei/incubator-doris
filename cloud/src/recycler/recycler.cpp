...
void Recycler::recycle_callback() {
    while (!stopped()) {
        ...
        auto instance_recycler =
                std::make_shared<InstanceRecycler>(txn_kv_, instance, txn_lazy_committer_);
...
int InstanceRecycler::do_recycle() {
    ...
    if (instance_info_.status() == InstanceInfoPB::DELETED) {
        ...
        if (advance_running_txn() != 0) ret = -1;
...
bool check_txn_abort(std::shared_ptr<TxnKv> txn_kv, const std::string& instance_id,
                     int64_t txn_id) {
    std::unique_ptr<Transaction> txn;
    TxnErrorCode err = txn_kv->create_txn(&txn);
    if (err != TxnErrorCode::TXN_OK) {
...
// Abort timeout txn or lazy commit txn
int InstanceRecycler::advance_running_txn() {
    const std::string task_name = "advance_running_txn";
    int num_scanned = 0;
    int num_timeout = 0;
    int num_abort = 0;
    ...
    auto handle_txn_running_kv = [&num_scanned, &num_timeout, &num_abort, &current_time, this](
        ...
        if (TxnStatusPB::TXN_STATUS_COMMITTED == txn_info.status()) {
            ...
            std::shared_ptr<TxnLazyCommitTask> task =
                    txn_lazy_committer_->submit_task(instance_id_, txn_id, txn_kv_);
            task->wait();
            txn_lazy_committer_->remove_task(txn_id);
            return 0;
        } else {
